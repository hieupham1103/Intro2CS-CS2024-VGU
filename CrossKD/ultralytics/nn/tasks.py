...
class BaseModel(nn.Module):
    ...
    def cross_forward(self, features):
        idx = -1
        cross_features = []
        cross_features.append(features[0]) # 经过第一个卷积后得到的特征图
        for m in self.model:
            if idx == -1:
                idx += 1
                continue
            ...
            if m.f != -1:
                x = cross_features[m.f] if isinstance(m.f,int) else [x if j == -1 else cross_features[j] for j in m.f]
            x = m(x)
            cross_features.append(x)
            idx += 1
        return cross_features

    def forward1(self, x, ir=torch.randn(1, 3, 512, 512), input_mode='RGB', augment=False, profile=False):
    ...
    def forward3(self, features):
        ...
        head = self.model[-1]
        x = features[head.f] if isinstance(head.f, int) else [features if j == -1 else features[j] for j in head.f]
        adjust_conv = [None for i in range(len(x))]
        for i in range(len(x)):
            ori_x = x[i]
            in_channels, out_channels = head.cv3[i][0].conv.in_channels, head.cv3[i][0].conv.out_channels
            adjust_conv[i] = nn.Conv2d(ori_x.shape[1], in_channels, kernel_size=3, stride=1, padding=1, bias=False).to(device)
        for i in range(len(x)):
            x[i] = adjust_conv[i](x[i])
        return head(x)[1]
    ...
    def forward_immediate_features(self, x):
        return self.predict_immediate_features(x)


    def predict(self, x, profile=False, visualize=False, augment=False, embed=None, GHOST=False, CrossKD=False, Cross=False):
        """
        ...
        if Cross==True:
            return self.cross_forward(x)
        if augment:
            return self._predict_augment(x)
        return self._predict_once(x, profile, visualize, embed)

    def predict_immediate_features(self, x):
        y, dt, immediate_features = [], [], []  # outputs
        for m in self.model:
            if m.f != -1:  # if not from previous layer
                x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]  # from earlier layers
            x = m(x)  # run
            y.append(x if m.i in self.save else None)  # save output
            immediate_features.append(x)
        return immediate_features
    ...
    def loss(self, batch, preds=None, Mode=None):
        """
        Compute loss.
        ...
        if Mode == 'KD':
            return self.forward_immediate_features(batch['img'])
