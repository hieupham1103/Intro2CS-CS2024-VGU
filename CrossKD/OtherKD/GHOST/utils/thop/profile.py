...
from GHOST.utils.thop.vision.basic_hooks import count_bn
...
def profile(model: nn.Module, inputs, custom_ops=None, verbose=True):
    ...
    def dfs_count(module: nn.Module, prefix="\t") -> (int, int):
        ...
        for m in module.children():
            ...
            if m in handler_collection and not isinstance(m, (nn.Sequential, nn.ModuleList)):#and not isinstance(nn.BatchNorm2d()):
                ...
                if type(m) in [nn.Conv1d,nn.Conv2d,nn.Conv3d]:
                    ...
                    ops_.append(m_ops) 
                    params_.append(m_params) 
                print(m,',',m_ops,',',m_params)
            else:
                m_ops, m_params = dfs_count(m, prefix=prefix + "\t")
                #print(m,m_ops,m_params)
            total_ops += m_ops
    ...
    return total_ops, total_params, ops, params,ops_,params_
