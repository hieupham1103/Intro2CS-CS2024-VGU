...
class ReviewKD(nn.Module):
    def __init__(
        self,  in_channels, out_channels, shapes, out_shapes,device='cuda'
    ):  
    ...
    def forward(self, s_features, t_features):
        ...
        #为了解决YOLOv8大模型和小模型特征图通道数不匹配的问题，引入特征对齐模块
        device = s_features[0].device
        adjust_conv = [None for i in range(len(results))]
        for i in range(len(results)):
            s = results[i]
            t = t_features[i]
            # in_channels, out_channels = t.shape[1], s.shape[1]
            # adjust_conv[i] = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False).to(device)
            # t_features[i] = adjust_conv[i](t_features[i])
            in_channels, out_channels = s.shape[1], t.shape[1]
            adjust_conv[i] = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False).to(device)
            results[i] = adjust_conv[i](results[i])
