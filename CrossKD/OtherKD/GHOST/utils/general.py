...
from OtherKD.GHOST.utils.google_utils import gsutil_getsize
from OtherKD.GHOST.utils.metrics import fitness
from OtherKD.GHOST.utils.torch_utils import init_torch_seeds
from OtherKD.GHOST.utils.ensemble_boxes import weighted_boxes_fusion
...
def xyxy2poly(x):
    # Convert nx4 boxes from [x1, y1, x2, y2] to [x, y, w, h] where xy1=top-left, xy2=bottom-right
    # y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y = torch.zeros(x.shape[0],8) if isinstance(x, torch.Tensor) else np.zeros(x.shape[0],8)
    y[:, 0] = x[:, 0]
    y[:, 1] = x[:, 1]
    y[:, 2] = x[:, 2]
    y[:, 3] = x[:, 1]
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    ...
    inter = (torch.min(b1_x2, b2_x2) - torch.max(b1_x1, b2_x1)).clamp(0) * \
            (torch.min(b1_y2, b2_y2) - torch.max(b1_y1, b2_y1)).clamp(0) #clamp(0)将小于0的元素修改为0，截断元素的取值空间
...
def unique_shape(s_shapes):
    ...
    for s_shape in s_shapes:
        if s_shape not in unique_shapes:
            ...
            # n += 1
        n_s.append(unique_shapes.index(s_shape))
    return n_s, unique_shapes

LAYER = {'SRyolo_noFocus': np.arange(0, 25-1),
         'SRyolo_noFocus_small': np.arange(0, 19-1),
         }

def define_shape(s_shapes):
    in_channels = []
    out_channels = []
    shapes = []
    out_shapes = []
    for s_shape in s_shapes:
        in_channels.append(s_shape[1])
        shapes.append(s_shape[2])
    out_channels = in_channels
    out_shapes = shapes
    return in_channels,out_channels,shapes,out_shapes
