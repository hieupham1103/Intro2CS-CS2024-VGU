...
class weight_quantize_fn(nn.Module):
  def __init__(self, w_bit):
    ...
    assert w_bit <= 8 or w_bit == 32 #todo
    self.w_bit = w_bit
  ...
  def forward(self, x):
    ...
    #   weight_q = 2 * self.uniform_q(weight,self.w_bit) - 1 #论文中没有乘以max_weight
      weight_q = 2 * Uniform_Quantize.apply(weight,self.w_bit) - 1
...
class activation_quantize_fn_relu(nn.Module):
  def __init__(self, a_bit):
    ...
    assert a_bit <= 8 or a_bit == 32 #todo
    self.a_bit = a_bit
    self.uniform_q = Uniform_Quantize.apply
  ...
  def forward(self, x):
    if self.a_bit == 32:
      ...
      # activation_q = self.uniform_q(torch.clamp(x, 0, 1),self.a_bit) #激活用relu
      activation_q = Uniform_Quantize.apply(torch.clamp(x, 0, 1), self.a_bit)  # 激活用relu
    #   activation_q = self.uniform_q(torch.clamp(x, -1, 1),self.a_bit-1)
    #   activation_q = self.uniform_q(x,self.a_bit)
...
class Quant_Conv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.full_precision_flag:
            ...
            w = self.weight
        # print(x.min(),x.max())
        # w_one_dim = self.weight.data.cpu().numpy().flatten()
