...
from  OtherKD.GHOST.utils.datasets import letterbox
from  OtherKD.GHOST.utils.general import non_max_suppression, make_divisible, scale_coords, xyxy2xywh
from  OtherKD.GHOST.utils.plots import color_list, plot_one_box
from  OtherKD.GHOST.utils.torch_utils import time_synchronized
from  OtherKD.GHOST.utils.activations import Mish
...
from  OtherKD.GHOST.utils.datasets import get_edge
...
class SSTN(nn.Module):
    ...
    def forward(self, x):  # B * C * H * W #x_left, x_right
        ...
        return torch.cat(
            [buffer_img, buffer_ir, feature_specific_img, feature_specific_ir, feature_shared_img, feature_shared_ir],
            1)
...
class SSFF1(nn.Module):
    ...
    def forward(self, x):  # B * C * H * W #x_left, x_right
        ...
        buffer_img_shared = self.b7(
            torch.cat([buffer_img_specific, feature_specific_img_hr, feature_specific_img_lr], 1))
        buffer_ir_shared = self.b7(torch.cat([buffer_ir_specific, feature_specific_ir_hr, feature_specific_ir_lr], 1))

        return torch.cat(
            [buffer_img_specific, buffer_ir_specific, buffer_img_shared, buffer_ir_shared, feature_specific_img_hr,
             feature_specific_ir_hr, feature_specific_img_lr, feature_specific_ir_lr], 1)
...
class ACmix(nn.Module):
    ...
    def forward(self, x):
        ...
        unfold_k = self.unfold(self.pad_att(k_att)).view(b * self.head, self.head_dim,
                                                         ...
                                                         w_out)  # b*head, head_dim, k_att^2, h_out, w_out #[4, 8, 49, 128, 128]
        unfold_rpe = self.unfold(self.pad_att(pe)).view(1, self.head_dim, self.kernel_att * self.kernel_att, h_out,
                                                        w_out)  # 1, head_dim, k_att^2, h_out, w_out #[1, 8, 49, 128, 128]
        ...
        f_all = self.fc(torch.cat(
            [q.view(b, self.head, self.head_dim, h * w), k.view(b, self.head, self.head_dim, h * w),
             v.view(b, self.head, self.head_dim, h * w)], 1))  # torch.Size([2, 6, 8, 16384]) [2, 9, 8, 16384]
        f_conv = f_all.permute(0, 2, 1, 3).reshape(x.shape[0], -1, x.shape[-2],
                                                   x.shape[-1])  # [2, 72, 128, 128] (2, -1, 128, 128)
